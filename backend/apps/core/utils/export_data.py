...
class AdminExportMixin:
    def format_filename(self, format):
        # Get the current datetime
        now = datetime.now().strftime("%Y%m%d%H%M%S")
        # Format the filename using the app label, model name and current datetime
        app_label = self.model._meta.app_label
        model_name = self.model._meta.model_name
        return f"{app_label}-{model_name}-{now}.{format}"
    ...
    def export_as_csv(self, request, queryset):
        ...
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = f'attachment; filename={self.format_filename("csv")}'
        writer = csv.writer(response)
    ...
    def export_as_json(self, request, queryset):
        ...
        response['Content-Disposition'] = f'attachment; filename={self.format_filename("json")}'
    ...
    def export_as_text(self, request, queryset):
        ...
        response['Content-Disposition'] = f'attachment; filename={self.format_filename("txt")}'
